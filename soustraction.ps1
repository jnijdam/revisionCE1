...
Function New-Question {
    ...
    if($script:count -le 10) {
        $tempNum1 = Get-Random -Minimum 0 -Maximum 20
        $tempNum2 = Get-Random -Minimum 0 -Maximum 20
        # Assurez-vous que num1 est toujours le plus grand
        if ($tempNum1 -ge $tempNum2) {
            $script:num1 = $tempNum1
            $script:num2 = $tempNum2
        } else {
            $script:num1 = $tempNum2
            $script:num2 = $tempNum1
        }
